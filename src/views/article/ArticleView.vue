                    ...
                    <el-table-column fixed="right" label="选项" align="center" width="300">
                        <template v-slot="scope" #default>
                            <el-button
                                link 
                                type="primary" 
                                v-if="scope.row.isCuration == 0"
                                @click="setCurationArticle(scope.row.articleId)"
                            >
...
import { queryAllArticle, setOrCancelCurationArticle } from '@/api/article';
...
function loadArticle() {
    queryAllArticle(articleQueryParams.value).then(response => {
...
// 设置精选文章
function setCurationArticle(articleId: any) {
    setOrCancelCurationArticle(articleId).then(response => {
        if (response.data.code === 200) {
