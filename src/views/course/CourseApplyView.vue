<template>
    <div class="course-apply-view-container">
        <el-card>
                    ...
                    <el-table-column fixed="right" label="选项" align="center" width="200">
                        <template #default="scope">
                            <el-button
                                link 
                                type="warning" 
                                @click="agreeCourse(scope.row.courseId)"
                            >
                                同意发布
                            </el-button>
                            <el-button
                                link 
                                type="danger" 
                                @click="unAgreeCourseDialog(scope.row.courseId)"
                            >
                                不同意发布
                            </el-button>
                        </template>
...
import { queryCourse, agreeReleaseCourse, unAgreeReleaseCourse } from '@/api/course';
...
function unAgreeApply() {
    ...
    unAgreeReleaseCourse(unAgreeApplyParams.value).then(response => {
        if (response.data.code === 200) {
...
// 同意发布
function agreeCourse(courseId: any) {
    console.log(courseId)
    agreeReleaseCourse(courseId).then(response => {
        if (response.data.code === 200) {
            ElMessage.success("操作成功")
            loadCourse()
        } else {
...
// 不同意发布
function unAgreeCourseDialog(courseId: any) {
    unAgreeApplyParams.value.courseId = courseId
    unAgreeDialog.value = true
}
</script>

<style lang="less" scoped>
.course-apply-view-container {
    height: 97%;
